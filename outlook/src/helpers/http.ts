...
export const postJsonRpc = async (
    relativeUrl: string,
    params: any
): Promise<any> => {
 ...
 * return false.
 */
export const isOdooDatabaseReachable = async (
    baseURL: string
): Promise<boolean> => {
    if (!baseURL) {
        return false
 ...
 */
export const openOdooLoginDialog = async (baseURL: string): Promise<string> => {
...
export function getOdooRecordURL(model, record_id) {
    ...
    return (
        baseUrl +
        `/mail_plugin/redirect_to_record/${model}/?record_id=${record_id}`
    )
}
