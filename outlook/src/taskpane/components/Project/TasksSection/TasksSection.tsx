...
class TasksSection extends React.Component<TasksSectionProps, TasksSectionState> {
    ...
    constructor(props, context) {
        ...
        this.state = {
            tasks: this.props.partner.tasks, isCollapsed: isCollapsed,
            isProjectCalloutOpen: false, key: Math.random()
        };
    }
    ...
    private createTask = (project: Project) => {
        Office.context.mailbox.item.body.getAsync(Office.CoercionType.Html, (result) => {
            ...
            const createTaskRequest = sendHttpRequest(HttpVerb.POST, api.baseURL + api.createTask,
                ContentType.Json,
                this.context.getConnectionToken(), taskValues, true);
            createTaskRequest.promise.then(response => {
    ...
    render() {
        ...
        if (!this.props.partner.isAddedToDatabase()) {
            if (!this.state.isCollapsed) {
                tasksExpanded = (
                    <div className="list-text">
                        {_t("Save Contact to create new Tasks.")}
                    </div>
                );
            }
        } else {
            if (!this.state.isCollapsed) {
                ...
                if (this.state.tasks.length > 0) {
                    ...
                    tasksContent = (<div className="list-text">
                        {_t("No tasks found for this contact.")}
                    </div>);
                }
                tasksExpanded = (
        ...
        if (this.state.tasks)
            title = _t("Tasks (%(count)s)", {count: this.state.tasks.length});
