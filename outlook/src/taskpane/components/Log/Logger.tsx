...
class Logger extends React.Component<LoggerProps, LoggerState> {
    ...
    private logRequest = async (event): Promise<any> => {
        ...
        Office.context.mailbox.item.body.getAsync(Office.CoercionType.Html, async (result) => {
            const msgHeader = `<div>${_t("From : %(email)s", {email: Office.context.mailbox.item.sender.emailAddress})}</div>`;
            const msgFooter = `<br/><div class="text-muted font-italic">${_t("Logged from")} <a href="https://www.odoo.com/documentation/14.0/applications/sales/crm/optimize/outlook_extension.html" target="_blank">${_t("Outlook Inbox")}</a></div>`;
            ...
            if (totalSize > SIZE_THRESHOLD_TOTAL * 1024 * 1024) {
                const warningMessage = _t("Warning: Attachments could not be logged in Odoo because their total size" +
            ...
            inlineAttachments.forEach((inlineAttachment) => {
                if (inlineAttachment != null && inlineAttachment.error == undefined) {
                    if (inlineAttachment.oversize) {
                        ...
                        imageElements[j].setAttribute("src",
            ...
            if (oversizeAttachments.length > 0) {
                ...
                const warningMessage = _t("Warning: Could not fetch the attachments %(attachments)s as their sizes are bigger then the maximum size of %(size)sMB per each attachment.",
                    {
