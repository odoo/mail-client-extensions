...
async function onNextLogin(
    ...
    let validatedUrl = formatUrl(formInputs.odooServerUrl);
    if (!validatedUrl) {
        return new Notify("Invalid URL");
    }
    ...
    } else if (validatedUrl.endsWith("/web")) {
        validatedUrl = validatedUrl.slice(0, -4);
    }

    user.odooUrl = formInputs.odooServerUrl;
    await user.save();

    const version = await getSupportedAddinVersion(validatedUrl);
    if (!version) {
    ...
    if (version !== 2) {
        return new Notify(
            "This addin version required Odoo 19.1 or a newer version, please install an older addin version.",
        );
    }
