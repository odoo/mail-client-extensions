...
export class User {
    ...
    /**
     * Check the token we receive from Google, and get the user base on the email.
     */
    ...
    static async getUserFromLoginToken(email: string, loginToken: string): Promise<User> {
        ...
        const compA = Buffer.from(loginToken);
        const compB = Buffer.from(user.loginToken);
        if (!crypto.timingSafeEqual(compA, compB)) {
            throw new Error("Invalid login token");
        }
