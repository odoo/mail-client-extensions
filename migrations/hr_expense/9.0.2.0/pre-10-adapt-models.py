...
def migrate(cr, version):
    product = (
        util.env(cr)["product.product"].with_context(active_test=False).search([("default_code", "=", "GEN_ODOO9_MIG")])
    )
    ...
    keep_sheets = pv(series) >= pv("9.saas~12") or util.str2bool(
        os.environ.get("ODOO_MIG_9_EXPENSES_KEEP_SHEETS", "yes")
    )
    keep_refs = []
    ...
    for dest_model, res_model, res_id in util.res_model_res_id(cr):
        ...
        if keep_sheets and dest_model in ["ir.attachment", "mail.followers", "mail.message"]:
            ...
            cr.execute("UPDATE {0} SET {1}=%s WHERE {1}='hr.expense.expense'".format(table, res_model), [temp_name])

        if not res_id:
            cr.execute("UPDATE {0} SET {1}='hr.expense' WHERE {1}='hr.expense.expense'".format(table, res_model))
        else:
    ...
    if keep_sheets:
        for table, res_model in keep_refs:
            cr.execute("UPDATE {0} SET {1}='hr.expense.expense' WHERE {1}=%s".format(table, res_model), [temp_name])
