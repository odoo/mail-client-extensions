...
def migrate(cr, version):
    ...
    count = cr.fetchone()[0]
    if count:
        util.announce(cr, version, 'Found %s stock.move entries with a Units of Measure incompatible with the default product UoM (different categories). Forcing them to the default product UoM' % count)
        cr.execute("""
