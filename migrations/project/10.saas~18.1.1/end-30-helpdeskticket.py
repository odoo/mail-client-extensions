...
def migrate(cr, version):
    if util.table_exists(cr, 'project_issue') and os.environ.get('ODOO_MIG_S18_NOSPLIT_PROJECTS'):
            ...
            cr.execute("""
                UPDATE ir_translation
                 ...
                 WHERE name = 'project.project,label_issues'
                   AND res_id IN %s
            """.format(json.dumps(created_teams)), [tuple(created_teams.keys())])
        ...
        columns = set(util.get_columns(cr, 'helpdesk_ticket', ignore)[0]) \
        ...
        """.format(columns=','.join(columns), select_columns=','.join(['i.%s' % c for c in columns]), ticket_type=helpdesk_ticket_type), [offset])
        ...
        cr.execute(
            "UPDATE mail_template SET model='helpdesk.ticket', model_id=%s WHERE model='project.issue'",
            [model_ticket]
        )
