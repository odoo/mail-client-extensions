...
class TestRefactoringEDI(TestRefactoringL10nMxEDI):
    ...
    def _prepare_test_invoice_with_payment(self):
        ...
        with self.with_mocked_pac_payment_sign_success():
            pay = self.env['account.payment.register'] \
                .with_context(active_model='account.move', active_ids=inv.ids).create({}) \
                ._create_payments()
        ...
        self.assertRecordValues(inv + pay.move_id, [
            {
    ...
    def _check_invoice_with_payment(self, _config, inv_id, pay_id, inv_cfdi_id, pay_cfdi_id):
        ...
        self.assertRecordValues(inv.l10n_mx_edi_invoice_document_ids, [
            {
                'invoice_ids': inv.ids,
