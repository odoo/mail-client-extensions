...
class TestRefactoringEDI(UpgradeCase):
    ...
    def _create_invoice_with_cfdi(self, user, invoice_vals):
        ...
        cfdi_str = self.env['ir.qweb']._render(self.env.ref('l10n_mx_edi.cfdiv33'), invoice._l10n_mx_edi_create_cfdi_values())
        ...
        cfdi = self.env['ir.attachment'].with_user(user).create({
            'name': filename,
            'res_id': invoice.id,
    ...
    def prepare(self):
        ...
        tax_16 = self.env['account.tax'].with_user(user).create({
            'name': 'tax_16',
            'amount_type': 'percent',
            'amount': 16,
            'type_tax_use': 'sale',
        ...
        product = self.env['product.product'].with_user(user).create({
            'name': 'product_mx',
            'weight': 2,
        ...
        partner = self.env['res.partner'].with_user(user).create({
            'name': 'partner_mx',
