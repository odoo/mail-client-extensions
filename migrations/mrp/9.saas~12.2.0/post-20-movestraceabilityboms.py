...
def migrate(cr, version):
    ...
    cr.execute("""
        INSERT INTO stock_move_lots (create_uid, create_date, product_id, done_wo, done_move,
                    production_id, move_id, quantity, quantity_done, lot_id)
        SELECT create_uid, create_date, product_id, 't', 't',
               COALESCE(raw_material_production_id, production_id), id, product_uom_qty,
               product_uom_qty, restrict_lot_id
        FROM stock_move
    ...
    cr.execute("""SELECT p.location_src_id, r.location_id
                ...
                AND p.location_src_id != r.location_id
                GROUP BY r.location_id, p.location_src_id""")
    res = cr.fetchall()
    if res and res[0]:
    ...
    SPT = env['stock.picking.type']
    domain = [('code', '=', 'internal')]
    picking_type_int = SPT.search(domain, limit=1)
    if not picking_type_int:
        picking_type_int = SPT.with_context(active_test=False).search(domain, limit=1)
    for source_location_id, location_id in res:
        ...
        env['procurement.rule'].create({
            'name': 'Make to Order %s to Production ' % (location_name,),
        ...
        env['procurement.rule'].create({
            'name': 'Before manufacturing from %s to %s' % (location_source_name, location_name),
