...
def migrate(cr, version):
    ...
    cr.execute("""
        INSERT INTO mrp_bom_line (type, product_id, product_uos_qty, product_uos, product_qty, product_uom, date_start, date_stop, sequence, 
        routing_id, product_rounding, product_efficiency, bom_id)
        (SELECT type, product_id, product_uos_qty, product_uos, product_qty, product_uom, date_start, date_stop, sequence, 
        routing_id, product_rounding, product_efficiency, bom_id FROM mrp_bom WHERE bom_id IS NOT NULL)
    """)
    
    # Delete all BoMs with a parent but without children
    cr.execute("""
        DELETE FROM mrp_bom WHERE bom_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM mrp_bom b2 where b2.bom_id = mrp_bom.id)
    """)
    ...
    cr.execute("""
    UPDATE mrp_bom SET product_tmpl_id = pp.product_tmpl_id FROM product_product pp WHERE mrp_bom.product_id = pp.id
    """)