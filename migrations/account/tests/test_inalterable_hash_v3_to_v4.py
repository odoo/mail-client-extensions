from odoo import fields
from odoo.tools import format_date

from odoo.addons.base.maintenance.migrations.testing import UpgradeCase, change_version
from odoo.addons.base.maintenance.migrations.util.accounting import no_fiscal_lock


@change_version("saas~17.2")
class TestAccountMoveInalterableHashV3ToV4Upgrade(UpgradeCase):
    def create_invoice(self, invoice_date, account, partner):
        return self.env["account.move"].create(
            {
                "move_type": "out_invoice",
                "partner_id": partner.id,
                "invoice_date": fields.Date.from_string(invoice_date),
                "invoice_line_ids": [
                    fields.Command.create(
                        {
                            "name": "Product",
                            "quantity": 1,
                            "price_unit": 100,
                            "account_id": account.id,
                        },
                    )
                ],
            }
        )

    def prepare(self):
        with no_fiscal_lock(self.env.cr):
            account = self.env["account.account"].create(
                {
                    "name": "Account",
                    "code": "ACC",
                    "account_type": "asset_current",
                }
            )
            partner = self.env["res.partner"].create({"name": "Partner"})
            moves = (
                self.create_invoice("2024-01-01", account, partner)
                | self.create_invoice("2024-01-02", account, partner)
                | self.create_invoice("2024-01-03", account, partner)
                | self.create_invoice("2024-01-04", account, partner)
                | self.create_invoice("2024-01-05", account, partner)
            )
            moves[0].journal_id.restrict_mode_hash_table = True
            moves.action_post()
        return {"move_ids": moves.ids, "account_id": account.id, "partner_id": partner.id}

    def check(self, result):
        moves = self.env["account.move"].browse(result["move_ids"])
        account = self.env["account.account"].browse(result["account_id"])
        partner = self.env["res.partner"].browse(result["partner_id"])
        moves.env.user.groups_id |= self.env.ref("account.group_account_user")

        # Check that the hashing chain check is still correct after the upgrade
        integrity_check = moves.company_id._check_hash_integrity()["results"]
        result = next(filter(lambda r: moves.journal_id.name in r.get("journal_name"), integrity_check))
        self.assertEqual(result["status"], "verified")
        self.assertEqual(result["first_move_date"], format_date(self.env, fields.Date.to_string(moves[0].date)))
        self.assertEqual(result["last_move_date"], format_date(self.env, fields.Date.to_string(moves[-1].date)))

        with no_fiscal_lock(self.env.cr):
            new_moves = self.create_invoice("2024-01-06", account, partner) | self.create_invoice(
                "2024-01-07", account, partner
            )
            moves |= new_moves
            new_moves.action_post()
            new_moves.button_hash()

        # Check that the hashing chain check is still correct after adding new moves
        integrity_check = moves.company_id._check_hash_integrity()["results"]
        result = next(filter(lambda r: moves.journal_id.name in r.get("journal_name"), integrity_check))
        self.assertEqual(result["status"], "verified")
        self.assertEqual(result["first_move_date"], format_date(self.env, fields.Date.to_string(moves[0].date)))
        self.assertEqual(result["last_move_date"], format_date(self.env, fields.Date.to_string(moves[-1].date)))
