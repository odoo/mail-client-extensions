...
def migrate(cr, version):
    ...
    # Handles 0% taxes for cash basis moves.
    cr.execute(
        """
        ...
        SELECT move.id, move.name, base_line.date, base_line.parent_state, base_line.journal_id, base_line.company_id,
               ...
               CASE WHEN account.id != tax.cash_basis_transition_account_id THEN atrl.id END,
               CASE WHEN account.id != tax.cash_basis_transition_account_id THEN tax.id END,
               CASE WHEN account.id != tax.cash_basis_transition_account_id THEN tax.tax_group_id END,
               TRUE,
               CASE WHEN account.id != tax.cash_basis_transition_account_id THEN
                   CASE
                       WHEN
        ...
        JOIN account_account account ON
            (
                tax.tax_exigibility = 'on_payment' AND account.id = tax.cash_basis_transition_account_id
            )
                OR
            (
                account.id = COALESCE(atrl.account_id, base_line.account_id)
            )
        ...
        WHERE NOT EXISTS (SELECT 1 FROM move_ids_to_exclude WHERE move_ids_to_exclude.move_id = base_line.move_id)
            AND move.tax_cash_basis_rec_id IS NOT NULL
        GROUP BY base_line.company_id, base_line.company_currency_id, base_line.currency_id, base_line.partner_id,
                 ...
                 COALESCE(NULLIF(ir_translation.value, ''), tax.name), tax.tax_exigibility, tax.type_tax_use,
                 CASE WHEN account.id != tax.cash_basis_transition_account_id THEN atrl.id END,
                 CASE WHEN account.id != tax.cash_basis_transition_account_id THEN tax.id END,
                 CASE WHEN account.id != tax.cash_basis_transition_account_id THEN tax.tax_group_id END,
                 atrl.invoice_tax_id, atrl.refund_tax_id,
                 tax.cash_basis_transition_account_id, account.id, CASE WHEN tax.analytic THEN base_line.analytic_account_id END

    """
    )

    # Handles regular 0% taxes for account move.
    cr.execute(
        """
        INSERT INTO account_move_line (move_id, move_name, date, parent_state, journal_id, company_id,
                                       company_currency_id, account_id, account_root_id, sequence, name,
        ...
        JOIN account_account account ON
            (
                ...
                OR
            (
                tax.tax_exigibility = 'on_invoice' AND account.id = COALESCE(atrl.account_id, base_line.account_id)
            )
        JOIN res_company ON move.company_id = res_company.id
        JOIN res_partner ON res_company.partner_id = res_partner.id
        LEFT JOIN ir_translation ON ir_translation.name = 'account.tax,name'
    ...
    cr.execute(
        ...
        INSERT INTO account_move_line (move_id, move_name, date, parent_state, journal_id, company_id,
                                       ...
                                       currency_id, tax_repartition_line_id, tax_line_id, tax_group_id, exclude_from_invoice_tab,
                                       tax_base_amount,
                                       group_tax_id, debit, credit, balance, amount_currency)
        SELECT move.id, move.name, base_line.date, base_line.parent_state, base_line.journal_id, base_line.company_id,
               base_line.company_currency_id, account.id, account.root_id, 10, COALESCE(NULLIF(ir_translation.value, ''), tax.name),
               base_line.partner_id, CASE WHEN tax.analytic THEN base_line.analytic_account_id END, account.reconcile, FALSE,
               base_line.currency_id, atrl.id, tax.id, tax.tax_group_id, TRUE,
