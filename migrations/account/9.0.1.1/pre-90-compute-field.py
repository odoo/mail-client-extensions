...
def migrate(cr, version):
    ...
    cr.execute("""UPDATE account_invoice inv
                    SET amount_total_company_signed = CASE WHEN inv.type IN ('in_refund', 'out_refund') THEN -inv.amount_total ELSE inv.amount_total END, 
                        amount_total_signed = CASE WHEN inv.type IN ('in_refund', 'out_refund') THEN -inv.amount_total ELSE inv.amount_total END,
                        amount_untaxed_signed = CASE WHEN inv.type IN ('in_refund', 'out_refund') THEN -inv.amount_untaxed ELSE inv.amount_untaxed END
                    FROM res_company c
