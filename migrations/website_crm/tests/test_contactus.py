...
class TestContactus(UpgradeCase):
    ...
    def check(self, init):
        ...
        self.assertEqual(get_view_wids(f"website.contactus{UPG_SUFFIX}"), set([website_2.id, website_4.id]))
        self.assertEqual(get_view_wids(f"website.contactus_form{UPG_SUFFIX}"), set([website_2.id, website_4.id]))
        self.assertEqual(
            get_view_wids(f"website_crm.contactus_form{UPG_SUFFIX}"),
            set([website_2.id, website_4.id, website_5.id]),
        )
        self.assertEqual(get_view_wids("website.contactus_thanks"), set([False]))
        self.assertEqual(get_view_wids(f"website.contactus_thanks{UPG_SUFFIX}"), set([website_1.id, website_2.id]))
        self.assertEqual(
        ...
        for website in Website.browse(init.values()):
            ...
            if website.id in (website_2.id, website_4.id):
                arch_1 = get_combined_arch(website.id, f"website.contactus{UPG_SUFFIX}")
            if website.id in (website_2.id, website_1.id):
                arch_2 = get_combined_arch(website.id, f"website.contactus_thanks{UPG_SUFFIX}")
            if website.id == website_2.id:  # this website COWed every views
                self.assertTrue(website.name in arch_1)
