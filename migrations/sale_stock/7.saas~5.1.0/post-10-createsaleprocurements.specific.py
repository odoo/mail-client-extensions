...
def _migrate(cr, version):
    ...
    property_by_partner = dict(
        [
            ...
            for r in partner_obj.read(cr, SUPERUSER_ID, [rec[0] for rec in shipping_ids], ["property_stock_customer"])
    ...
    state_by_procurement = dict(
        (k, [v["state"] for v in itr])
        for k, itr in groupby(sorted(cr.dictfetchall(), key=itemgetter("procurement_id")), itemgetter("procurement_id"))
    )
    ...
    for line_ids in util.log_progress(
        util.chunks(sol_dict.keys(), chunk_size, list), qualifier=qual, logger=_logger, size=size
    ):
        cr.execute(sql_sale_order_line_read, [tuple(line_ids)])
        lines = cr.dictfetchall()
        for line in lines:
            cr.execute(sql_sale_order_read, [line["order_id"]])
            ...
            vals.update({"move_ids": related_moves, "rule_id": rule, "state": proc_state})
...
def insert_procurement_order(cr, vals):
    ...
    sql = """\
        ...
        ", ".join(cols), ", ".join(["%({})s".format(c) for c in cols])
    )
    cr.execute(sql, vals)
    res = cr.fetchone()
    proc_id = res[0]
...
def _prepare_order_line_procurement(cr, order, line, group_id, property_by_partner, security_lead_by_company):
    security_lead = security_lead_by_company[order["company_id"]]
    date_planned = datetime.datetime.strptime(order["date_order"], DEFAULT_SERVER_DATETIME_FORMAT) + datetime.timedelta(
        days=line["delay"] or 0.0
    )
    date_planned = (date_planned - datetime.timedelta(days=security_lead)).strftime(DEFAULT_SERVER_DATETIME_FORMAT)
