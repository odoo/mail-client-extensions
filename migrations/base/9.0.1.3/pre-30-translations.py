...
def migrate(cr, version):
    ...
    if view_ids:
        cr.execute("""
          ...
          GROUP BY lang
        """)
        langs = [lg[0] for lg in cr.fetchall()]
        ...
        cr.execute(
            ...
            SELECT v.id, t.path, v.{} FROM ir_ui_view v JOIN view_tree t ON (v.id = t.id)
        ...
        for view, view_path, arch in cr.fetchall():
            ...
            def cb(term, view=view, view_path=view_path):
                for lang in langs:
    ...
    cr.execute(
        """INSERT INTO ir_translation(name, type, res_id, {columns})
