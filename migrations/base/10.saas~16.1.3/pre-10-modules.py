...
def migrate(cr, version):
    ...
    util.new_module(cr, 'l10n_be_hr_payroll_fleet', ('l10n_be_hr_payroll', 'fleet'),
                    auto_install=True)
    util.new_module(cr, 'mass_mailing_event', ('event', 'mass_mailing'), auto_install=True)
    util.new_module(cr, 'mass_mailing_event_track', ('mass_mailing', 'website_event_track'),
                    auto_install=True)
    util.new_module(cr, 'website_portal_purchase', ('website_portal', 'purchase'),
                    auto_install=True)
    ...
    if util.module_installed(cr, 'website_portal'):
        ...
        util.rename_xmlid(
            cr, *util.expand_braces('{,website_}portal.mail_template_data_portal_welcome'))

    util.remove_module(cr, 'portal')

    # portal stock
    util.rename_xmlid(cr, 'portal_stock.portal_stock_picking_portal_user_rule',
                          'stock.stock_picking_rule_portal')
    util.rename_xmlid(cr, 'portal_stock.portal_stock_pack_operation_portal_user_rule',
                          'stock.stock_pack_operation_rule_portal')
    ...
    if util.has_enterprise():
        ...
        util.new_module(
            cr, 'hr_contract_salary',
            ('hr', 'website', 'hr_recruitment', 'l10n_be_hr_payroll_fleet', 'website_sign'),
            auto_install=True)

        util.new_module(cr, 'project_timesheet_forecast_sale',
                        ('project_forecast', 'sale_timesheet'),
                        auto_install=True)
