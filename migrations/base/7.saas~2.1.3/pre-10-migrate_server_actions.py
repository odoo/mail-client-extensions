...
def migrate(cr, v):
    ...
    cr.execute("SELECT id, name, email, subject, message, model_id FROM ir_act_server WHERE state=%s", ('email',))
    ...
    for aid, name, email_to, subject, message, model_id in email_actions:
        ...
        cr.execute("""INSERT INTO email_template(name, model, model_id, subject, email_from, email_to, body_html)
                                          VALUES(%s, %s, %s, %s, %s, %s, %s)
                      RETURNING id
                   """, (name, model_name, model_id, to_jinja(subject or ''), email_from, email_to, to_jinja(message or '')))
