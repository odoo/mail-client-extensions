...
def migrate(cr, version):
    ...
    cr.execute(
        ...
        INSERT INTO sale_order_template (old_template_id,name,active,note,recurring_rule_type,
             ...
             SELECT sst.id,CONCAT ('Upgraded ', sst.name),active,description,recurring_rule_type,
                    recurring_rule_boundary,recurring_rule_count,user_closable,auto_close_limit,
                    good_health_domain,bad_health_domain,invoice_mail_template_id,company_id,str.id
               FROM sale_subscription_template sst
    ...
    query = """
        INSERT INTO sale_order (old_subscription_id,name,campaign_id,source_id,medium_id,client_order_ref,
                                ...
                                company_id,partner_id,partner_invoice_id,partner_shipping_id,recurring_monthly,
             ...
             SELECT ss.id,'mig '||ss.name||'-'||ss.id,campaign_id,source_id,medium_id,ss.code,
                    ...
                    ss.date_start,ss.recurring_next_date %s
               FROM sale_subscription ss
               JOIN sale_order_template sot ON sot.old_template_id=ss.template_id
               JOIN product_pricelist pl ON pl.id=ss.pricelist_id
               JOIN sale_temporal_recurrence str ON ss.template_id = ANY(str._mig_sst_id)
    """
    insert_add = ""
    select_add = ""
