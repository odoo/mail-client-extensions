...
def migrate(cr, version):
    ...
    if util.version_between("17.0", "19.0"):
        if util.module_installed(cr, "hr_recruitment_sms"):
            util.rename_xmlid(cr, *eb("hr_recruitment{,_sms}.action_hr_applicant_mass_sms"), on_collision="merge")
        else:
    ...
    if util.version_between("18.0", "saas~18.1"):
        if util.module_installed(cr, "hr_recruitment_sms"):
            ...
            util.rename_xmlid(cr, *eb("hr_recruitment{,_sms}.action_hr_candidate_mass_sms"), on_collision="merge")
        else:
