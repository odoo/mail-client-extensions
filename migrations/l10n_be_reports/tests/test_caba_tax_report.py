...
class TestCABATaxReport(UpgradeCase):
    ...
    def prepare(self):
        ...
        for test_tax in test_taxes:
            ...
            self.create_invoice(company, 100, test_tax, '2015-01-04', inv_line_account, pay=True, refund=True, negate_half=True)
            ...
            refund_wizard = self.env['account.invoice.refund'].with_context(active_ids=inv_to_refund.ids, active_id=inv_to_refund.id, active_model='account.invoice').create({
                'description': "test refund",
                'date': inv_to_refund.date,
            ...
            aml_to_unrec = inv_to_unrec.move_id.line_ids.filtered(lambda x: x.account_id.user_type_id.type in ('payable', 'receivable'))
        ...
        mig_check_data['tax_report_by_date']['2015-01-05'] = {'03': 100, '54': 21, '59': 21, '49': 100, '64': 21, '85': 100, '63': 21}
        mig_check_data['tax_report_by_date']['2015-01-06'] = {'03': 100, '54': 21, '59': 21, '49': 100, '64': 21, '85': 100, '63': 21}

        return mig_check_data

    def create_invoice(self, company, price_unit, tax, date, inv_line_account, refund=False, pay=False, negate_half=False):
        """ Returns an open invoice """

        invoice_type = (tax.type_tax_use == 'sale' and 'out_' or 'in_') + (refund and 'refund' or 'invoice')
        #rec_account_type = tax.type_tax_use == 'sale' and self.env.ref('account.data_account_type_receivable') or self.env.ref('account.data_account_type_payable')
        #rec_account = self.env['account.account'].search([('company_id', '=', company.id), ('user_type_id', '=', rec_account_type.id)], limit=1)
    ...
    def check(self, init):
        ...
        for date, predicted_dict in init['tax_report_by_date'].items():
            report_opt = self.env['account.generic.tax.report']._get_options({'date': {'period_type': 'custom', 'filter': 'custom', 'date_to': date, 'mode': 'range', 'date_from': date}})
            ...
            for tag_name, total in predicted_dict.items():
                report_line = self.env['account.tax.report.line'].search([('country_id', '=', company.country_id.id), ('tag_name', '=', tag_name)])
                self.assertEqual(total, report_dict[report_line.id], "Cash basis tax entry not migrated properply for tag_name %s, with case at date %s" % (tag_name, date))
