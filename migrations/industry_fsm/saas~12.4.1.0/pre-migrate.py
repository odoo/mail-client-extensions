...
def migrate(cr, version):
    util.create_column(cr, "project.task", "fsm_state", "varchar")
    cr.execute(
        """
        UPDATE project_task t
           SET fsm_state = CASE WHEN t.fsm_done AND t.sale_line_id IS NOT NULL
                                     AND f.allow_billable AND p.timesheet_product_id IS NOT NULL
                                THEN 'sold'
                                WHEN t.fsm_done THEN 'validated'
                                ELSE 'draft'
                            END
    ...
    util.remove_field(cr, "project.task", "fsm_done")
